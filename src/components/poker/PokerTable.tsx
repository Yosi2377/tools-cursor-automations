...
const PokerTable: React.FC<PokerTableProps> = ({ roomId, onLeaveRoom }) => {
  ...
  const prevGameContextRef = useRef(gameContext);
  const prevCurrentPlayerRef = useRef<Player | null>(null);

  // Memoize the current player to prevent unnecessary re-renders
  const currentPlayer = useMemo(() => {
    if (!gameContext.players.length) return null;
    const player = gameContext.players.find(p => p.isTurn);
    if (JSON.stringify(player) === JSON.stringify(prevCurrentPlayerRef.current)) {
      return prevCurrentPlayerRef.current;
    }
    prevCurrentPlayerRef.current = player || null;
    return player || null;
  }, [gameContext.players]);
  ...
  // Handle bot actions with debounce
  useEffect(() => {
    if (
      !gameContext.gameId || 
      gameContext.gameState !== 'betting' ||
    ...
    // Compare with previous game context to prevent unnecessary updates
  ...
  // Handle community card dealing with debounce
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [gameContext, dealCommunityCards]);
  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-b from-[#1a1a2e] to-[#16213E] p-2 sm:p-3 md:p-4 overflow-hidden">
      ...
      <div className="relative w-full h-full flex flex-col items-center justify-center">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black/50 text-white px-4 py-2 rounded z-30">
          Pot: ${potSize}
        </div>
