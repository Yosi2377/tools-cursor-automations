    ...
    while (joinAttempts < MAX_JOIN_ATTEMPTS) {
        try {
            ...
            const element = await Promise.race([
                page.waitForSelector('.game-table-container', { timeout: JOIN_TIMEOUT }),
            ...
            const elementType = await element.evaluate(el => {
                if (el.classList.contains('game-table-container')) return 'table';
                return 'button';
            });
            ...
            await page.waitForSelector('.game-table-container', { timeout: JOIN_TIMEOUT });
            ...
            try {
                ...
                const gameTableExists = await page.evaluate(() => {
                    return !!document.querySelector('.game-table-container');
                });

                if (gameTableExists) {
                    console.log('✅ Found game table after recovery check');
                    return true;
                }
