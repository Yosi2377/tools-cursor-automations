...
const PlayerSpot: React.FC<PlayerSpotProps> = ({ player, onTimeout }) => {
  ...
  const getCardPositionClasses = () => {
    switch (player.position) {
      case 'bottom':
        return 'top-full mt-6';
      case 'bottomRight':
        return '-top-1/2 -translate-y-full -mt-4';
      case 'right':
        return '-left-full -translate-x-full -mt-4';
      case 'topRight':
        return 'bottom-full translate-y-1/2 -mb-4';
      case 'top':
  ...
  return (
    ...
    >
      <div className="relative">
        <div className="w-20 h-20 rounded-full overflow-hidden border-3 border-gray-600/80">
      ...
      <div className="mt-3 text-center">
        ...
        >
          <p className="text-white font-bold text-base mb-1 drop-shadow-lg">{player.name || 'Empty Seat'}</p>
      ...
      {player.cards.length > 0 && (
        <div className={`absolute left-1/2 transform -translate-x-1/2 mt-1 flex gap-1 ${getCardPositionClasses()}`}>
