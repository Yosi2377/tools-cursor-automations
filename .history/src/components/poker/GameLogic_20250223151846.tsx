...
export const useGameLogic = (
  ...
  const checkGameState = () => {
    ...
    console.log('Game state check:', { allPlayersActed });
    ...
    if (allPlayersActed) {
      ...
      // Set a timeout to force game progression
      setTimeout(forceGameProgression, FORCE_PROGRESS_DURATION);
    }
  };
  ...
  const handleOpponentAction = async (
    ...
    // Set a timeout for the player action
    ...
    try {
      ...
      clearTimeout(timeoutId); // Clear the timeout if action is completed
    }
  };

  const handleFold = async (player: Player) => {
    console.log(`${player.name} has folded.`);
    // Update player state to reflect fold
    setGameContext(prev => {
