    ...
    const initializeGame = async () => {
      try {
        ...
        if (room) {
          ...
          const totalPlayers = room.with_bots ? 8 : room.actual_players; // 8 bots + 1 player when bots enabled
          ...
          if (room.with_bots) {
            ...
            const { data: game, error: gameError } = await supabase
              ...
              .insert([{
                room_id: roomId,
                status: 'betting',
                current_player_index: 1, // Start with the first bot
                pot: 0,
            ...
            const { error: playersError } = await supabase
              ...
              .insert(
                players.map((p, index) => ({
                  ...
                  is_active: true,
                  current_bet: 0,
                  is_turn: index === 1 // First bot's turn
                }))
              );
