    ...
    const initializeGame = async (roomId: string, gameId?: string) => {
      try {
        ...
        const { data: botIds, error: botError } = await supabase.rpc('generate_bot_uuids', {
          total_players: room.max_players - 1
        });
        ...
        const players = Array.from({ length: room.max_players }, (_, index) => {
          ...
          return {
            id: playerId,
            name: isCurrentPlayer ? user?.email?.split('@')[0] || 'Player' : `Bot ${index}`,
            chips: 1000,
            position: index,
            isBot: !isCurrentPlayer,
            folded: false,
            currentBet: 0,
          };
        });
        ...
        const parsedCommunityCards = game.community_cards ? JSON.parse(game.community_cards) : [];
        ...
        setGameContext({
          ...
          currentPlayer: players[game.current_player_index],
