...
export const useGameState = (roomId: string) => {
  ...
  const getPositionForIndex = (index: number, totalPlayers: number): PlayerPosition => {
    ...
    // If we have less than max players, distribute them evenly
    if (totalPlayers < positions.length) {
      const spacing = Math.floor(positions.length / totalPlayers);
      const offset = Math.floor((positions.length - (totalPlayers * spacing)) / 2);
      return positions[(index * spacing + offset) % positions.length];
    }
