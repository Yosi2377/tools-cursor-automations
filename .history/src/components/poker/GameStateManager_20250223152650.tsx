  ...
  useEffect(() => {
    const initializeGame = async () => {
      try {
        ...
        const mergedPlayers = updatedPlayers.map(player => {
          player.name = player.name || 'Unnamed'; // Set default name if undefined
          player.chips = player.chips || 0; // Set default chips if undefined
          player.isActive = true; // Mark all players as active
          return player;
        });
