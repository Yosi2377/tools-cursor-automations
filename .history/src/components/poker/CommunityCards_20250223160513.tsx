    ...
    <div className="relative flex items-center justify-center gap-3">
        ...
        {cards.map((card, index) => (
          <motion.div
            ...
            initial={{ 
              ...
              y: -150,
              rotateY: 180,
              x: (index - 2) * 60 // Reduced spread between cards
            }}
            animate={{ 
              scale: 1,
              opacity: 1,
          ...
          >
            ...
            >
              ...
              <PlayingCard
                ...
                className="card w-14 h-20 sm:w-16 sm:h-24 md:w-20 md:h-28 rounded-xl shadow-2xl" // Reduced card size
              />
      ...
      {Array.from({ length: 5 - cards.length }).map((_, index) => (
        <motion.div
          ...
          className="w-[10%] aspect-[5/7] rounded-xl border-2 border-dashed border-white/30 bg-gradient-to-br from-white/5 to-transparent backdrop-blur-sm"
          style={{
            transform: `translateX(${(index + cards.length - 2) * 60}px)` // Adjusted spacing
          }}
        >
          {/* Slot inner glow */}
