...
const TableLayout: React.FC<TableLayoutProps> = ({ gameContext, onTimeout }) => {
  ...
  const getPlayerPosition = (position: PlayerPosition) => {
    switch (position) {
      ...
      case 'bottomRight':
        return 'bottom-[60%] right-[40%] translate-x-0';
      case 'right':
        return 'top-1/2 right-[40%] -translate-y-1/2 translate-x-0';
      case 'topRight':
        return 'top-[60%] right-[40%] translate-x-0';
      case 'top':
        return 'top-[40%] left-1/2 -translate-x-1/2';
      case 'topLeft':
        return 'top-[60%] left-[40%] translate-x-0';
      case 'left':
        return 'top-1/2 left-[40%] -translate-y-1/2 translate-x-0';
      case 'bottomLeft':
        return 'bottom-[60%] left-[40%] translate-x-0';
      default:
        return '';
    }
  };
  ...
  const getCardPosition = (position: string) => {
    switch (position) {
      ...
      case 'right':
        return '-left-full -translate-x-8 -mt-4';
      case 'topRight':
        return 'top-1/2 -translate-y-1/2 -mt-8';
      case 'top':
        return '-top-full -mt-8';
      case 'topLeft':
        return 'top-1/2 -translate-y-1/2 -mt-8';
