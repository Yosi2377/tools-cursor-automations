...
export const useGameState = (roomId: string): GameStateManager => {
  ...
  }, [gameContext.players, gameContext.communityCards.length, dealNextCommunityCards, updateGameState]);
  ...
  useEffect(() => {
    const initializeGame = async () => {
      ...
      try {
        ...
        if (!botIds) throw new Error('Failed to generate bot IDs');
  ...
  useEffect(() => {
    if (!gameContext.gameId || !gameContext.isInitialized) return;
      ...
      .on(
        ...
        (payload: GameSubscriptionPayload) => {
          ...
          updateGameState(prev => ({
            ...
            currentBet: newGameState.current_bet || prev.currentBet,
            pot: newGameState.pot || prev.pot,
            rake: newGameState.rake || prev.rake,
            dealerPosition: newGameState.dealer_position || prev.dealerPosition,
    ...
    gameSubscription.current = channel;

    return () => {
      if (gameSubscription.current) {
        void gameSubscription.current.unsubscribe();
      }
    };
  }, [gameContext.gameId, gameContext.isInitialized, updateGameState]);
