...
export interface GameLogicHook {
  ...
  dealNextCommunityCards: () => Promise<void>;
}
...
export const useGameLogic = (
  ...
  const dealCards = async (): Promise<void> => {
    try {
      ...
      await dealNextCommunityCards(3); // Ensure this function is called correctly
      console.log('Game context after dealing cards:', gameContext);
    } catch (error) {
  ...
  const dealCommunityCards = async (): Promise<void> => {
    ...
    console.log('Checking if all players have acted:', { allPlayersActed, currentBet: gameContext.currentBet });
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        current_player_index: (gameContext.dealerPosition + 1) % gameContext.players.length,
        status: stage === 'River' ? 'showdown' : 'betting',
        current_bet: 0
      })
      .eq('id', gameContext.gameId);
    ...
    updateGameState(prev => ({
      ...prev,
      gameState: stage === 'River' ? 'showdown' : 'betting',
    ...
    console.log('Dealing community cards, current game context:', gameContext);
  };
