...
export const useGameState = (roomId: string) => {
  ...
  }, [gameContext.players, dealNextCommunityCards]);

  // Initialize game state when component mounts
  useEffect(() => {
    const initializeGame = async (roomId: string, gameId?: string): Promise<void> => {
      try {
        ...
        if (!existingPlayers || existingPlayers.length === 0) {
          // Insert new players
          const { error: playerError }: { error: unknown } = await supabase
            .from('game_players')
