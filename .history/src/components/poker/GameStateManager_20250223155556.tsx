      ...
      try {
        ...
        if (playersError) {
          ...
          const updatedPlayers = initialPlayers.map(player => {
            ...
            console.log('Updated player before merge:', player);
            const updatedPlayer = fetchedPlayer ? {
              ...player,
              chips: fetchedPlayer.chips,
              cards: typeof fetchedPlayer.cards === 'string' ? JSON.parse(fetchedPlayer.cards) : [],
            ...
            // Ensure all players are initialized correctly
            updatedPlayer.name = updatedPlayer.name || 'Unnamed'; // Set default name if undefined
            updatedPlayer.chips = updatedPlayer.chips || 0; // Set default chips if undefined
            updatedPlayer.isActive = true; // Mark all players as active
            return updatedPlayer;
          });
          console.log('Updated players after merging:', updatedPlayers);
          setGameContext(prev => ({
            ...prev,
            players: updatedPlayers,
            gameId: game?.id,
            gameState: (game?.status as GameState) || "waiting",
            currentBet: game?.current_bet || 0,
            pot: game?.pot || 0,
            rake: game?.rake || 0,
            dealerPosition: game?.dealer_position || 0,
            currentPlayer: game?.current_player_index || 0,
            communityCards: game?.community_cards ? JSON.parse(game.community_cards as string) : [],
            isInitialized: true
          }));
        }
