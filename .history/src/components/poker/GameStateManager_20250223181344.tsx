      ...
      try {
        ...
        if (game) {
          setGameContext(prev => ({
            ...prev,
            gameId: game.id,
            players: [], // Assuming players need to be fetched separately
            gameState: game.status as GameState,
            currentBet: game.current_bet,
            pot: game.pot,
            communityCards: Array.isArray(game.community_cards) ? game.community_cards : (typeof game.community_cards === 'string' ? JSON.parse(game.community_cards) : []),
        ...
        const initialPlayers: Player[] = Array(totalPlayers).fill(null).map((_, index) => {
          ...
          return {
            id: isHumanPlayer ? user.id : botIds[index],
