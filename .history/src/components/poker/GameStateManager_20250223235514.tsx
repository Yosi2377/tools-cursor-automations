...
import { GameContext as GameContextType } from '@/types/poker';
...
export const useGameState = (roomId: string): GameStateManager => {
  ...
  const updateGameState = useCallback((updater: (prevState: GameContext) => GameContext) => {
    setGameContext(prev => {
      ...
      console.log('Community cards after update:', next.communityCards);
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    const initializeGame = async () => {
      ...
      try {
        ...
        console.log('Community cards after fetching:', game.community_cards);
          ...
          .on(
            ...
            (payload: { new: GameRow }) => {
              ...
              if (
                ...
                (newGame.community_cards && JSON.stringify(JSON.parse(newGame.community_cards)) !== JSON.stringify(prevGameContext.current.communityCards))
              ) {
                updateGameState(prev => ({
                  ...prev,
                  ...
                  communityCards: newGame.community_cards ? JSON.parse(newGame.community_cards) as Card[] : [],
                }));
              }
            }
          )
      ...
      .on(
        ...
        async (payload: PlayerSubscriptionPayload) => {
          try {
            ...
            // Ensure updatedPlayer properties are correctly typed
            const updatedPlayerData: Player = {
              ...
              hasActed: false,
              hasFolded: false,
              stack: 1000, // Assuming a default stack value
              isVisible: true, // Ensure isVisible is set
              position: updatedPlayer.position as PlayerPosition // Ensure position is of type PlayerPosition
            };
  ...
  const gameContext = useContext(GameContextType);
