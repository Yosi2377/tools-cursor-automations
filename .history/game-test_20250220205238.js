...
async function handlePlayerTurn(page) {
    try {
        ...
        if (gameState.allPlayersActed && !gameState.hasFlop) {
            console.log('🃏 Waiting for flop...');
            try {
                await page.waitForSelector('.card', { timeout: 15000 }); // Increased from 10000 to 15000
                await page.waitForTimeout(3000); // Added extra delay after flop
            } catch (error) {
                console.log('⚠️ Timeout waiting for flop');
                throw new Error('Flop not dealt after all players acted');
            }
        }
