    ...
    while (joinAttempts < MAX_JOIN_ATTEMPTS) {
        try {
            ...
            if (elementType === 'table') {
                // Verify the poker table is properly initialized
                const tableState = await page.evaluate(() => {
                    const table = document.querySelector('[data-testid="poker-table"]');
                    if (!table) return { state: 'missing' };
                    
                    // Check for error messages
                    const errorElement = table.querySelector('.error-message');
                    if (errorElement) {
                        return { state: 'error', message: errorElement.textContent || 'Unknown error' };
                    }
                    ...
                    if (!hasGameTable || !hasPlayers || !hasControls) {
                        return { 
                            state: 'incomplete', 
                            missing: { 
                    ...
                    // Check for loading/initialization states
                    const loadingElement = table.querySelector('.loading-message');
                    if (loadingElement) {
                        return { state: 'initializing', message: loadingElement.textContent || 'Loading...' };
                    }
                ...
                if (tableState.state === 'ready') {
                    console.log('✅ Poker table is ready');
                    return true;
                } else if (tableState.state === 'error') {
                    console.log(`⚠️ Poker table error: ${tableState.message}`);
                    await sleep(2000);
                    continue;
                    ...
                    console.log('⚠️ Poker table is incomplete:', tableState.missing);
                    await sleep(2000);
                    continue;
                }
            }
            ...
            // Wait for poker table to appear and initialize
            const tableElement = await page.waitForSelector('[data-testid="poker-table"]', { timeout: JOIN_TIMEOUT });
