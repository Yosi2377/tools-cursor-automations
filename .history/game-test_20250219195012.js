...
async function analyzeScreenshotAndDecideAction(page, screenshotName) {
    ...
    try {
        ...
        // Analyze the screenshot and return decision
        return analyzeGameState(page);
    } catch (error) {
...
async function joinGame(page, roomId) {
    ...
    while (joinAttempts < MAX_JOIN_ATTEMPTS) {
        try {
            ...
            const { action, button, errors, screenshot, state } = await analyzeScreenshotAndDecideAction(page, `join_attempt_${joinAttempts + 1}`);
            ...
            if (action === 'JOIN' && lastAction !== 'JOIN' && button) {
                ...
                const verifyState = await analyzeScreenshotAndDecideAction(page, 'join_verification');
            ...
            if (action === 'START_HAND' && lastAction !== 'START_HAND' && button) {
                ...
                const verifyState = await analyzeScreenshotAndDecideAction(page, 'hand_start_verification');
                if (verifyState.errors && verifyState.errors.length > 0) {
            ...
            const { state, screenshot } = await analyzeScreenshotAndDecideAction(page, `error_state_${joinAttempts}`);
