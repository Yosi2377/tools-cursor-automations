...
async function handlePlayerTurn(page) {
    try {
        ...
        if (action === 'PLAY_TURN' && button) {
            ...
            // Check if all players have acted and cards should be dealt
            const gameState = await page.evaluate(() => {
                const playerSpots = document.querySelectorAll('[data-testid="player"]');
                const allPlayersHaveBet = Array.from(playerSpots).every(spot => {
                    const betElement = spot.querySelector('.current-bet');
                    return betElement && parseFloat(betElement.textContent.replace('$', '')) > 0;
                });
            ...
            if (gameState.allPlayersHaveBet && !gameState.hasCards) {
