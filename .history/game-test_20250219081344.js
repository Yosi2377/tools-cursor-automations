...
async function joinGame(page, roomId) {
    ...
    while (joinAttempts < MAX_JOIN_ATTEMPTS) {
        try {
            ...
            const initialState = await analyzeScreenshot(page, 'before_join');
            console.log('📊 Initial state analysis:', initialState?.elements?.buttons?.length, 'buttons found');
            ...
            // After clicking join, analyze state again
            const afterJoinState = await analyzeScreenshot(page, 'after_join');
            console.log('📊 Post-join state:', 
                afterJoinState?.elements?.pokerTable ? '✅ Table found' : '❌ No table',
                afterJoinState?.elements?.players, 'players');

            if (afterJoinState?.elements?.errors?.length > 0) {
                console.error('⚠️ Errors found:', afterJoinState.elements.errors);
                throw new GameStateError('Errors detected after join', {
                    errors: afterJoinState.elements.errors
                });
            }
            ...
            await analyzeScreenshot(page, `join_error_attempt_${joinAttempts}`);
