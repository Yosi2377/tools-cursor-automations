    ...
    while (joinAttempts < MAX_JOIN_ATTEMPTS) {
        try {
            ...
            if (gameTable) {
                ...
                if (!playersPresent) {
                    console.error('⚠️ No players found after joining the game.');
                    throw new Error('No players found after joining game');
                }
                return true;
            }
...
async function handlePlayerActions(page) {
    ...
    // Check for players waiting for their turn to expire
    const playersWaiting = await page.evaluate(() => {
        const playerSpots = document.querySelectorAll('[data-component-file="PlayerSpot.tsx"]');
        return Array.from(playerSpots).some(spot => {
            const isActive = spot.classList.contains('active');
            const isTurn = spot.classList.contains('turn');
            return isActive && isTurn;
        });
    });

    if (playersWaiting) {
        console.log('⚠️ Detected players waiting for their turn to expire. Attempting to resolve...');
        // Automatically trigger an action to resolve the waiting state
        const activePlayerSpot = await page.evaluate(() => {
            const activeSpot = document.querySelector('.border-emerald-500');
            return activeSpot ? activeSpot : null;
        });
        if (activePlayerSpot) {
            console.log('🔄 Taking action for waiting player...');
            await page.click('button:has-text("FOLD")'); // Example action to force next turn
            await page.waitForTimeout(3000); // Wait for the action to process
        }
    }
    ...
    console.error('🚨 Timeout reached while waiting for flop to be dealt.');
}

async function runTest() {
    let browser, context, page;
    let gameInProgress = true; // Flag to track game progress
    ...
    try {
        ...
        if (gameStarted) {
            ...
            // Continuously handle player actions until the game ends or an error occurs
            while (gameInProgress) {
                try {
                    ...
                    gameInProgress = false; // Stop the game loop on error
                }
            }
        }
