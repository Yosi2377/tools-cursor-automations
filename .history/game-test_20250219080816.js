...
async function joinGame(page, roomId) {
    ...
    while (joinAttempts < MAX_JOIN_ATTEMPTS) {
        try {
            ...
            // Try to start a new game immediately after joining
            ...
            if (startButton) {
                ...
                console.log('⚠️ Start game button not found, checking for alternative buttons...');
                
                // Check for "start new hand" button as an alternative
            ...
            try {
                ...
                if (gameTableExists) {
                    ...
                    // Try to start game even in recovery
                    const startButton = await page.$('button:has-text("Start Game")');
                    if (startButton) {
                        console.log('🎮 Starting new game during recovery...');
                        await startButton.click();
                        await page.waitForTimeout(2000);
                    }
