...
async function joinGame(page, roomId) {
    ...
    while (joinAttempts < MAX_JOIN_ATTEMPTS) {
        try {
            ...
            // Check if game table is visible and centered
            ...
            if (tablePosition) {
                ...
                if (!tablePosition.visible || 
                    tablePosition.y < 0 || 
                    tablePosition.y > window.innerHeight ||
                    tablePosition.x < 0 || 
                    tablePosition.x > window.innerWidth) {
                    console.log('⚠️ Table is not fully visible, adjusting viewport...');
                    await page.evaluate(() => {
                        const table = document.querySelector('.game-table-container');
                        if (table) {
            ...
            if (action === 'PLAY_TURN' || (state && state.cards && state.cards.length > 0 && state.players && state.players.length > 0)) {
            ...
            // Instead of reloading, try to fix the table position
            await page.evaluate(() => {
                const table = document.querySelector('.game-table-container');
                if (table) {
                    table.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' });
