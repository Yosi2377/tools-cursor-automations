...
// Helper function to save screenshot both as file and base64
async function saveScreenshot(page, name) {
    ...
    // Also get base64 for logging
    const buffer = await page.screenshot({ fullPage: true });
    const base64Image = buffer.toString('base64');
    
    // Save analysis with the screenshot
    const analysisPath = `${filename}.analysis.txt`;
    const state = await analyzeCurrentState(page);
    await fs.promises.writeFile(analysisPath, JSON.stringify(state, null, 2));
    
    console.log(`📸 Saved screenshot: ${filename}`);
    console.log('🔍 Current page state:', state);
    
    return { filename, base64Image, state };
}

// Helper function to analyze current page state
async function analyzeCurrentState(page) {
    return await page.evaluate(() => {
        const state = {
            ...
            errors: [],
            consoleMessages: [],
            gameTablePresent: false,
            playerCount: 0,
            cardCount: 0
        };

        // Check for visible buttons
        state.visibleButtons = Array.from(document.querySelectorAll('button'))
            .filter(b => {
                const rect = b.getBoundingClientRect();
        ...
        // Check for game table
        const gameTable = document.querySelector('.game-table-container');
        state.gameTablePresent = !!gameTable;
        
        // Count players and cards
        state.playerCount = document.querySelectorAll('[data-testid="player"]').length;
        state.cardCount = document.querySelectorAll('.card').length;

        // Check for errors
        state.errors = Array.from(document.querySelectorAll('.error, .error-message'))
            .map(e => e.textContent);
...
async function runTest() {
    ...
    try {
        ...
        // Login
        await login(page);
        const { state: loginState } = await saveScreenshot(page, 'after_login');
        console.log('Login state:', loginState);
        
        // Join game
        const gameStarted = await joinGame(page, 'roomId');
        const { state: joinState } = await saveScreenshot(page, 'after_join');
        ...
        if (gameStarted) {
            ...
            if (action !== 'PLAY_TURN') {
                ...
                const { state: gameState } = await saveScreenshot(page, 'game_started');
                console.log('Game state:', gameState);
            } else {
                console.log('✅ Game already in progress, skipping new hand start');
                const { state: currentState } = await saveScreenshot(page, 'game_in_progress');
        ...
        if (page) {
            const { state: errorState } = await saveScreenshot(page, 'fatal_error');
